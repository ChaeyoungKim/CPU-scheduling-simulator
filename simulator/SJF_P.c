...
void SJF_P(Process* process, Queue* ready_queue, Queue* waiting_queue, int n, double* WT3, double* TT3) {
	...
	*TT3 = avg_turnaround_time;
	...
	for (t = 0; t < 500; t++) { //put processes in the ready queue in the order of arrival time.
		for (i = 0; i < n; i++) { //if there are processes that arrvied at t, put them in the ready queue
		...
		if (!isEmpty(ready_queue)) {
			if (ready_queue->array[ready_queue->out].io_start_time == 0) {
				...
				//printf("\nready queue : "); showQueue(ready_queue);
				if (!isEmpty(ready_queue)) {
					sort_by_cpuburst(ready_queue); //if sorted right after processes arrive, it could happen that process is moved back in line and can't go to waiting queue. So sorting needed right before CPU burst.
				...
				sort_by_cpuburst(ready_queue); //if sorted in advance, it could be moved back in line and not go to waiting queue, so it's sorted right before CPU burst.
				printf("P%d(%d) ", ready_queue->array[ready_queue->out].process_id, t);
				ready_queue->array[ready_queue->out].cpu_burst_time--;
				ready_queue->array[ready_queue->out].io_start_time--;
				if (ready_queue->array[ready_queue->out].cpu_burst_time == 0) {
		...
		} //some processes in waiting queue
	} //for loop. time.
	printf("\n\n");
